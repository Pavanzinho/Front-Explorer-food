import { Container } from "./style";
import { RedButton } from "../RedButton";
import { useNavigate } from "react-router-dom";
import { useEffect } from "react";
import { useState } from "react";
import { api } from "../../service/api";


export function AdmFoodCard({ data, onClick, className }) {
    const navigate = useNavigate()
    /*     const [plate, setPlate] = useState([]); */
    /*  const avatarUrl=data.avatar? `${api.defaults.baseURL}/files/${data.avatar}`: ""; */

    // console.log(data)


    return (
        <Container className={className}>
            <button
                id="handleDetails"
                onClick={onClick}
            >
                <div className="foodImg">
                    <img src={`${api.defaults.baseURL}/files/${data.avatar}`} alt="" />
                </div>
                <svg id="pencil" width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M13.7514 4.81033L21.1896 12.2486M7.99997 25H2.06262C1.7808 25 1.51052 24.888 1.31124 24.6888C1.11196 24.4895 1.00001 24.2192 1.00001 23.9374V18C0.999523 17.8621 1.02626 17.7253 1.07868 17.5977C1.1311 17.4701 1.20818 17.354 1.30551 17.2562L17.2447 1.31701C17.3436 1.21661 17.4614 1.13687 17.5914 1.08245C17.7214 1.02803 17.8609 1 18.0018 1C18.1427 1 18.2822 1.02803 18.4122 1.08245C18.5422 1.13687 18.66 1.21661 18.7589 1.31701L24.683 7.24107C24.7834 7.33995 24.8631 7.45781 24.9175 7.58779C24.972 7.71777 25 7.85727 25 7.99819C25 8.1391 24.972 8.27861 24.9175 8.40859C24.8631 8.53857 24.7834 8.65642 24.683 8.7553L8.7438 24.6945C8.64599 24.7918 8.52994 24.8689 8.40231 24.9213C8.27467 24.9737 8.13795 25.0005 7.99997 25Z" stroke="#E1E1E6" trokewidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>

                {/*colocar uma condicional aq, trocar fill no svg em vermelho e */}

                <span className="foodTitle">
                    {data.plate_title}
                    <svg width="4" height="7" viewBox="0 0 12 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fillRule="evenodd" clipRule="evenodd" d="M0.292893 0.293122C0.683417 -0.0974022 1.31658 -0.0974022 1.70711 0.293122L11.7071 10.2931C12.0976 10.6836 12.0976 11.3168 11.7071 11.7073L1.70711 21.7073C1.31658 22.0979 0.683417 22.0979 0.292893 21.7073C-0.0976311 21.3168 -0.0976311 20.6836 0.292893 20.2931L9.58579 11.0002L0.292893 1.70734C-0.0976311 1.31681 -0.0976311 0.683646 0.292893 0.293122Z" fill="white" />
                    </svg>

                </span>

                <span className="descriptionOfFood">
                    {data.plate_description}
                </span>

                <div className="priceOfFood">
                    <span>
                        R$
                    </span>

                    <span>
                        {data.plate_price}
                    </span>

                </div>
            </button>
        </Container>
    )
}